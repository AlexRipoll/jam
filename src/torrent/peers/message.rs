...
impl PiecePayload {
    ...
    pub fn deserialize(payload: &[u8]) -> Result<Self, MessageError> {
...
// Implement `std::fmt::Display` for `MessageError`
impl Display for MessageError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MessageError::BufferTooShort => write!(f, "Buffer too short for length prefix"),
            MessageError::PayloadTooShort => write!(f, "Payload too short for deserialization"),
        }
    }
}

// Implement `std::error::Error` for `MessageError`
